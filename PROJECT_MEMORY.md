...
## Current electrical revision — R2-MULTISHEET

The current source of truth is the seven-sheet KiCad project generated by `generate_kicad_multisheet.py`. Any older Rev-1 pin tables or power descriptions later in this file are historical and are superseded by `SCHEMATIC_GUIDE.md`.

Current architecture:

- Fused 3S input keeps Pi/ESP logic alive while a real external J18 latching E-stop loop removes `ACT_VBAT` from wheels and servos.
- Raspberry Pi has a dedicated approximately 5.1 V / 5 A TPS54560 rail and J9 Mini-Fit Jr power connector.
- Pi communication uses J10, a normal 2.54 mm 1x8 header. ESP UART is GPIO18/19, not the USB programming UART. Pi GPIO17 supplies default-off `ACTUATOR_ENABLE`.
- Left motor: PWM25, INA26, INB27, current ADC36, encoder 34/35, diagnostic GPIO23.
- Right motor: PWM14, INA13, INB4, current ADC39, encoder 37/38, diagnostic GPIO5.
- Motor diagnostics are separate at the ESP and diode-aggregated for the Pi/status LED.
- J4/J5 order is red motor+, black encoder ground, yellow A, green B, blue filtered 3.3 V, white motor−. J15/J16 are the preferred large red/white power-wire pads.
- PCA9685 uses I2C GPIO21/22 and OE GPIO32. J12/J13/J14 receive independent PWM channels.
- J12 Miuzei main swing uses 8.4 V; J13 MG996R and J14 MG90S use 6 V.
- Ordinary small passives are 0805; power-output MLCCs are intentionally 1210; bulk capacitors and mini-blade fuse holders are through-hole.
- Current schematic ERC result: zero errors and zero warnings. PCB layout has not started.
- Remaining design gates are hardware 3S undervoltage protection, exact MLCC/connector mechanical selection, and first-board regulator/current/thermal validation.
